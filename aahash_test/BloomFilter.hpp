...
{
public:
    BloomFilter(size_t filterSize, unsigned hashNum, unsigned kmerSize, const char *fPath) : m_size(filterSize), m_hashNum(hashNum), m_kmerSize(kmerSize)
    {
    ...
    BloomFilter(size_t filterSize, unsigned hashNum, unsigned kmerSize) : m_size(filterSize), m_hashNum(hashNum), m_kmerSize(kmerSize)
    {
        m_filter = new unsigned char[(m_size + 7) / 8];
    ...
    {
        uint64_t hVal = btllib::aahash_base(kmer, m_kmerSize, 1);
        size_t hLoc = hVal % m_size;
        __sync_or_and_fetch(&m_filter[hLoc / 8], (1 << (7 - hLoc % 8)));
        for (unsigned i = 1; i < m_hashNum; i++)
        {
            uint64_t mhVal = hVal * (i ^ m_kmerSize * btllib::MULTISEED);
            mhVal ^= mhVal >> btllib::MULTISHIFT;
            size_t hLoc = mhVal % m_size;
            __sync_or_and_fetch(&m_filter[hLoc / 8], (1 << (7 - hLoc % 8)));
        }
    }

    void insert(const char *kmer, uint64_t &hVal)
    {
        hVal = btllib::aahash_base(kmer, m_kmerSize, 1);
        size_t hLoc = hVal % m_size;
        __sync_or_and_fetch(&m_filter[hLoc / 8], (1 << (7 - hLoc % 8)));
        for (unsigned i = 1; i < m_hashNum; i++)
        {
    ...
    void insert(uint64_t &hVal, const char charOut, const char charIn)
    {
        hVal = btllib::aahash_roll(hVal, m_kmerSize, charOut, charIn, 1);
        size_t hLoc = hVal % m_size;
        __sync_or_and_fetch(&m_filter[hLoc / 8], (1 << (7 - hLoc % 8)));
        for (unsigned i = 1; i < m_hashNum; i++)
        {
    ...
    bool contains(const char *kmer, uint64_t &hVal)
    {
        hVal = btllib::aahash_base(kmer, m_kmerSize, 1);
        size_t hLoc = hVal % m_size;
        if ((m_filter[hLoc / 8] & (1 << (7 - hLoc % 8))) == 0)
    ...
    bool contains(uint64_t &hVal, const char charOut, const char charIn)
    {
        hVal = btllib::aahash_roll(hVal, m_kmerSize, charOut, charIn, 1);
        size_t hLoc = hVal % m_size;
        if ((m_filter[hLoc / 8] & (1 << (7 - hLoc % 8))) == 0)
